...
async function searchMatches(query) {
  ...
  try {
    ...
    if (data.response && data.response.length > 0) {
      console.log(
        `${data.response.length} results found for search query: ${query}`
      );
      displayMatches(data.response);
    } else {
      document.getElementById(
        "liveMatches"
      ).innerHTML = `<p>No matches found for "${query}".</p>`;
    }
  } catch (error) {
    console.error("Error searching matches:", error);
    document.getElementById(
      "liveMatches"
    ).innerHTML = `<p>Error searching for matches.</p>`;
  }
}
...
function generateDateSelector() {
  ...
  for (let i = -1; i <= 1; i++) {
    ...
    div.onclick = () =>
      fetchLiveMatches(null, date.toISOString().split("T")[0]);
  }
}
...
async function populatePinnedLeagues() {
  ...
  try {
    ...
    if (data.response && data.response.length > 0) {
      const activeLeagues = data.response.filter((league) =>
